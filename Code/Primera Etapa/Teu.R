...
------------------------------------------------------------------------------------------------------------------
#11. Modelo: Salario base de cotización diario de trabajadores eventuales urbanos asegurados (continua, sal_bas_cotdi_teu) respecto a Asegurados (continua, asegurados)
...
#Descripción de variables
asgChi_teu$asegurados
str(asgChi_teu$asegurados)
summary(asgChi_teu$asegurados)
table(asgChi_teu$asegurados)
asgChi_teu$sal_bas_cotdi_teu
str(asgChi_teu$sal_bas_cotdi_teu)
summary(asgChi_teu$sal_bas_cotdi_teu)
table(asgChi_teu$sal_bas_cotdi_teu)
cor(asgChi_teu$asegurados, asgChi_teu$sal_bas_cotdi_teu)

#Modelo
#Lineal
m11 <- lm(sal_bas_cotdi_teu ~ asegurados, asgChi_teu)
m11
summary(m11)
#Cuadrático
asegurados2 <- asgChi_teu$asegurados^2
m112 <- lm(sal_bas_cotdi_teu ~ asegurados + asegurados2, asgChi_teu)
m112
summary(m112)
#Cúbico
asegurados3 <- asgChi_teu$asegurados^3
m113 <- lm(sal_bas_cotdi_teu ~ asegurados + asegurados2 + asegurados3, asgChi_teu)
m113
summary(m113)
#Potencia 4
asegurados4 <- asgChi_teu$asegurados^4
m114 <- lm(sal_bas_cotdi_teu ~ asegurados + asegurados2 + asegurados3 + asegurados4, asgChi_teu)
m114
summary(m114)
...
#Lineal
x <- seq(min(asgChi_teu$asegurados), max(asgChi_teu$asegurados), by = 0.01)
y <- 152.8422 - 0.1849 * x
#Cuadrático
x2 <- x^2
y2 <- 158.603607 - 0.676556 * x + 0.004247 * x^2
#Cúbico
x3 <- x^3
y3 <- 1.611e+02 - 1.057e+00 * x + 1.252e-02 * x^2 - 4.119e-05 * x^3
#Potencia 4
x4 <- x^4
y4 <- 1.625e+02 - 1.374e+00  * x + 2.518e-02 * x^2 - 1.932e-04 * x^3 + 5.435e-07 * x^4

#Gráficación
plot(asgChi_teu$asegurados, asgChi_teu$sal_bas_cotdi_teu)
...
#Descripción del modelo
	Modelo que describe a la variable sal_bas_cotdi_teu respecto a la variable asegurados. La primera es
	una variable continua o númerica que describe el salario base de cotización (SBC) que refiere a la razón de 
	la masa salarial o nómina y el número de asegurados asociados a un empleo, en este caso a empleos eventuales 
	urbanos. La segunda es una variable continua o númerica que describe el número de personas que están aseguradas 
	en el IMSS bajo un patrón.
	
	Las métricas el modelo son:
		1. La correlación del modelo es de -0.1313768.
		3. El error descrito por RMSE del modelo lienal es 45.8, cuádratico es 45.66, cúbico es 45.87, de potencia 4 es 46.14.
		4. El coeficiente de determinación ajustado del modelo lienal es 0.005275, cuádratico es 0.01155, cúbico es 0.002313, de potencia 4 es -0.009466.
		5. Solo b0 es significativo.
		
	El coeficiente de correlación tiene signo negativo por lo que la realción entre variables en inversamente proporcional.
	La magnitud del mismo es considerablemente baja. El coeficiente de determinación ajustado es bajo, también. Solo b0 es 
	significante. Cunado el salario base de cotización para empleos eventuales urbanos aumenta, el número de asegurados bajo 
	un mismo patrón disminuye.
	
	Es decir, mientas más alto es el salario base de cotización de un asegurado, para empleos eventuales urbanos, menos 
	asegurados tiene ese mismo patrón. Sin embargo, al no existir una relación fuerte entre variables, y ya que las metricas 
	indican una relación débil, la conclusión anterior no es tan plausible, pues hay asegurados con un salario base de cotización 
	igual, sin importar, que su patron cuente con 1 asegurado o con más de 150. Por ello las predicciónes resultantes del modelo 
	no son, ni de cerca, exactas. El modelo no sigue un modelo lineal, ni cuadrático, ni cúbico, ni a la potencia 4, por más que 
	mejoren las métricas, aunque el modelo cuadrático, cúbico y de potencia podrían producir buenas aproximaciones a 
	predicciones.
	
------------------------------------------------------------------------------------------------------------------
#12. Modelo: Salario base de cotización diario de trabajadores eventuales urbanos asegurados (continua, sal_bas_cotdi_teu) respecto a Masa salarial de trabajadores asegurados (continua, masa_sal_ta)
...
#Modelo
#Lineal
m12 <- lm(sal_bas_cotdi_teu ~ masa_sal_ta, asgChi_teu)
m12
summary(m12)
#Cuadrático
masa_sal_ta2 <- asgChi_teu$masa_sal_ta^2
m122 <- lm(sal_bas_cotdi_teu ~ masa_sal_ta + masa_sal_ta2, asgChi_teu)
m122
summary(m122)
#Cúbico
masa_sal_ta3 <- asgChi_teu$masa_sal_ta^3
m123 <- lm(sal_bas_cotdi_teu ~ masa_sal_ta + masa_sal_ta2 + masa_sal_ta3, asgChi_teu)
m123
summary(m123)
#Potencia 4
masa_sal_ta4 <- asgChi_teu$masa_sal_ta^4
m124 <- lm(sal_bas_cotdi_teu ~ masa_sal_ta + masa_sal_ta2 + masa_sal_ta3 + masa_sal_ta4, asgChi_teu)
m124
summary(m124)
...
------------------------------------------------------------------------------------------------------------------
#13. Modelo: Salario base de cotización diario de trabajadores eventuales urbanos asegurados (continua, sal_bas_cotdi_teu) respecto a Sexo (categórica, sexo) y Tamaño de registro patronal (categórica, tama.o_patron)
...
#Modelo
#Lineal
m13 <- lm(sal_bas_cotdi_teu ~ sexo_bin + tama.o_patron_num, asgChi_teu)
m13
summary(m13)
...
------------------------------------------------------------------------------------------------------------------
#14. Modelo: Salario base de cotización diario de trabajadores eventuales urbanos asegurados (continua, sal_bas_cotdi_teu) respecto a Asegurados (categórica, sexo) y Masa salarial de trabajadores asegurados eventuales urbanos (continua, masa_sal_teu)

#Descripción de variables
cor(asgChi_teu$asegurados, asgChi_teu$masa_sal_teu)
cor(asgChi_teu$asegurados, asgChi_teu$sal_bas_cotdi_teu)
cor(asgChi_teu$masa_sal_teu, asgChi_teu$sal_bas_cotdi_teu)

#Modelo
#Lineal
m14 <- lm(sal_bas_cotdi_teu ~ asegurados + masa_sal_teu, asgChi_teu)
m14
summary(m14)

#Cálculo de estimaciones
#Lineal
xa <- seq(min(asgChi_teu$asegurados), max(asgChi_teu$asegurados), length.out = 100)
xm <- seq(min(asgChi_teu$masa_sal_teu), max(asgChi_teu$masa_sal_teu), length.out = 100)
y <- 145.68685 - 0.47913 * xa + 0.05065 * xm

#Gráficación
plot((asgChi_teu$asegurados + asgChi_teu$masa_sal_teu), asgChi_teu$sal_bas_cotdi_teu)
lines(xa+xm, y, col = "blue", lwd = 2)
		
#Descripción del modelo
	Modelo que describe a la variable sal_bas_cotdi_teu respecto a la variable asegurados y masa_sal_teu. La primera es 
	una variable continua o númerica que describe el salario base de cotización (SBC) que refiere a la razón de la masa 
	salarial o nómina y el número de asegurados asociados a un empleo, en este caso a empleos eventuales urbanos. La segunda 
	...
	Las métricas el modelo son:
		1. La correlación entre variables independientes es 0.653363. Entre sal_bas_cotdi_teu y asegurados es -0.1313768, y entre sal_bas_cotdi_teu y masa_sal_teu es 0.09753303.
		3. El error descrito por RMSE del modelo lineal es 44.68.
		4. El coeficiente de determinación ajustado del modelo lineal es 0.05311.
	...
	Cunado hay un cambio en el número de asegurados por un mismo patrón, el cambio al salario será en una proporción de -0.47913. 
	...
	al salario será en un proporción de 0.05065.
